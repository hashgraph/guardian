...
create_iam_role_policy() {
  aws iam create-role --role-name my-role --assume-role-policy-document file://$ROLE_DIR
  ...
  for POLICY in $POLICY_DIR/*; do
    ...
    aws iam attach-role-policy --role-name my-role --policy-arn arn:aws:iam::486330131759:policy/$POLICY_NAME
  done

...
context("Contracts",{ tags: ["policy_labels", "formulas", "trustchains", "contracts", "firstPool", "all"] },() => {
    ...
    describe("Create and approve retire request without approve", () => {
      ...
      it("Verify balance decreased", () => {
        getWithAuth(SRUsername, apiBase + API.ListOfTokens).then((response) => {
          expect(response.status).to.eq(STATUS_CODE.OK);
          const found = (response.body || []).find((t) => t.tokenId === tokenId);
          expect(found, "Token exists to verify balance").to.exist;
          expect(found.balance).to.eq("7");
        });

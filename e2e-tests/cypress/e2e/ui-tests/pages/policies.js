...
export class PoliciesPage {
    ...
    importPolicyFile(file) {
        ...
        cy.get(PoliciesPageLocators.importFile).selectFile('@myFixture', { force: true });
        cy.get(PoliciesPageLocators.continueImportBtn).click();
    }

    publishPolicy() {
        cy.intercept(PoliciesPageLocators.policiesList).as('waitForPoliciesList')
        cy.wait('@waitForPoliciesList', { timeout: 180000 }).then(() => {
            ...
            cy.get('tbody>tr').eq('0')
              ...
              .then(() => {
                  cy.wait(5000)
                cy.contains(PoliciesPageLocators.dropDawnPublishBtn).click({ force: true }).then(() => {
                cy.get(PoliciesPageLocators.versionInput).type('0.0.1').then(() => {
                  cy.contains(PoliciesPageLocators.publishPolicyBtn, 'Publish').click().then(() => {

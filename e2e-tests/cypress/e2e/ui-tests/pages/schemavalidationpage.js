...
export class SchemaValidationPage {
    ...
    checkrequiredcondition(fieldName, errormessage, value) {
        ...
        cy.get('[type="file"]').selectFile("@myFixture", {
            ...
            cy.contains(fieldName).parentsUntil('.form-field-container').find('input').type(value);
        }
...
var title = 'Applicant Details'
        ...
        if (incorrectValue !== "NO VALIDATION") {
            cy.contains(fieldName).parentsUntil('.form-field-container').find('input').type(incorrectValue); 
            cy.wait(500) 
...
//enter correct value
        ...
        cy.get('[type="file"]').selectFile("@myFixture", {
            ...
            cy.contains(fieldName).parentsUntil('.form-field-container').find('input').clear().type(value);
        }
...
//multiple 
    ...
    checkMultipleInputrequiredcondition(fieldName, errormessage, value) {
        ...
        if (value == true) {
            ...
            cy.contains(fieldName).parentsUntil('.form-field-container').find('.add-btn').click();

 ...
 *
 * NOTE: this is a copy of `common/src/helpers/cryppo/index.ts`, which is the canonical version.
 * The two trees have separate dependency graphs (the indexer frontend does not link Guardian's
 * `common`), so the code is duplicated on purpose - keep the two files in sync. Only the decrypt
 * path is exercised here; the indexer is read-only and never encrypts. The generated fixture
 * vectors in both test suites (`cryppo.fixtures.json` here) pin the shared wire format.
 *

import { PolicyModel, SchemaModel, TokenModel, FileModel } from '../models/index.js';
import { IWeightBlock, IWeightItem, IPolicyData, CompareOptions, IChildrenLvl, IEventsLvl, IIdLvl, IKeyLvl, IPropertiesLvl, IRefLvl } from '../interfaces/index.js';
...
export class HashComparator {
     ...
     * @static
     */
    public static async createHashMap(policy: PolicyModel) {
        if (!policy) {
            return { hash: null, hashMap: null };
        }
        const tree = HashComparator.createTree(policy);
        const hash = CompareUtils.sha256(JSON.stringify(tree));
        return { hash, hashMap: tree };
    }
     ...
     * @static
     */
    public static compare(policy1: IPolicy, policy2: IPolicy): number {

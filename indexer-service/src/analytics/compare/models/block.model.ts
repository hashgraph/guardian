...
import { CompareOptions, IChildrenLvl, IPropertiesLvl, IArtifacts, IKeyMap, IWeightModel, IWeightBlock, IBlockRawData } from '../interfaces/index.js';
...
export class BlockModel implements IWeightModel {
    ...
    public update(options: CompareOptions): void {
        ...
        if (this._prop) {
            hash3.clear();
            hash3.add(this.key)
            hash3.add(this._prop.hash(options));
            _prop = hash3.result();
        }

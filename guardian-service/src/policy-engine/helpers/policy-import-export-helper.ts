...
export class PolicyImportExportHelper {
    ...
    public static addBlock(parent: any, block: any) {
     ...
     * @param policy
     */
    public static async addPolicyTools(policy: Policy, tools: PolicyTool[]): Promise<Policy> {
        const toolIds = new Set<string>()
        PolicyImportExportHelper.findTools(policy.config, toolIds);
        for (const tool of tools) {
            if (!toolIds.has(tool.messageId)) {
                toolIds.add(tool.messageId);
                PolicyImportExportHelper.addBlock(policy.config, {
                    id: GenerateUUIDv4(),
                    tag: 'Autogenerated',

...
function Policies() {
    ...
    let policy, policyMessageId, policyImportObject;
    ...
    it('/policies/{policyId}/export', async function() {
        ...
        policyMessageId = result.data.messageId;
        delete result.data.messageId
        assert.deepEqual(result.data, { name: 'e2e', version: '1.0.0'});
    });
    ...
    it('/policies/import/preview', async function() {
        ...
        result = await axios.post(
            ...
            {messageId: '1644249666.780502057'},
            {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${GetToken('RootAuthority')}`,

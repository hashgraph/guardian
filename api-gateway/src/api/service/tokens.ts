...
export class TokensApi {
    ...
    async updateTokenAsync(@Req() req, @Response() res): Promise<any> {
        ...
        try {
            ...
            const taskManager = new TaskManager();
            const task = taskManager.start('Update token', user.id);
            RunFunctionAsync<ServiceError>(async () => {
    ...
    async deleteTokenAsync(@Req() req, @Response() res): Promise<any> {
        ...
        try {
            ...
            const taskManager = new TaskManager();
            const task = taskManager.start('Delete token', user.id);
            RunFunctionAsync<ServiceError>(async () => {
    ...
    async associateTokenAsync(@Req() req, @Response() res): Promise<any> {
        ...
        const taskManager = new TaskManager();
        const task = taskManager.start(TaskAction.ASSOCIATE_DISSOCIATE_TOKEN, user.id);
        RunFunctionAsync<ServiceError>(async () => {
            const guardians = new Guardians();
    ...
    async grantKycOld(@Req() req, @Response() res): Promise<any> {
        ...
        try {
            ...
            const result = await guardians.grantKycToken(tokenId, username, owner, req.user.id);
    ...
    async grantKyc(@Req() req, @Response() res): Promise<any> {
        ...
        try {
            ...
            return res.json(await guardians.grantKycToken(tokenId, username, userDid, req.user.id));
        } catch (error) {
            new Logger().error(error, ['API_GATEWAY']);
            if (error?.message?.toLowerCase().includes('user not found')) {
                throw new HttpException('User not found', HttpStatus.NOT_FOUND)
    ...
    async grantKycAsyncOld(@Req() req, @Response() res): Promise<any> {
        ...
        const taskManager = new TaskManager();
        const task = taskManager.start(TaskAction.GRANT_REVOKE_KYC, user.id);
        RunFunctionAsync<ServiceError>(async () => {
            const guardians = new Guardians();
    ...
    async freezeTokenAsync(@Req() req, @Response() res): Promise<any> {
        ...
        const taskManager = new TaskManager();
        const task = taskManager.start('Freeze token', user.id);
        RunFunctionAsync<ServiceError>(async () => {
            const guardians = new Guardians();
    ...
    async unfreezeTokenAsync(@Req() req, @Response() res): Promise<any> {
        ...
        const taskManager = new TaskManager();
        const task = taskManager.start('Unfreeze token', user.id);
        RunFunctionAsync<ServiceError>(async () => {
            const guardians = new Guardians();

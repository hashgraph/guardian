...
<template>
    ...
    >
        <!-- Panel: hard-locked to 320px (w-80). overflow-hidden clips any
             ...
             `scrollbar-gutter: stable` (set inline so older Tailwind builds
             without the arbitrary-property plugin still apply it) reserves
             the vertical scrollbar's ~15px gutter even when no scroll is
             needed — without it the inner content width was ~320px for
             short countries (Peru) but ~305px for tall countries (India,
             many sectors), so the panel APPEARED to widen/narrow per
             country. -->
        <div
            v-if="detail"

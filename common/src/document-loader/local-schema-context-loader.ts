...
export class LocalSchemaContextLoader extends DocumentLoader {
    /**
     * Resolved schema contexts, keyed by context URL. Published contexts are
     * immutable, so a short TTL removes the per-signature DB lookup.
     */
    ...
    public async getDocument(iri: string): Promise<any> {
        return LocalSchemaContextLoader.cache.getOrLoad(iri, async () => {

...
describe('resolveGeoDependencies', () => {
    it('resolves a parent named by its key', () => {
        ...
        XlsxToJson.resolveGeoDependencies(worksheet, table, level, result);
    ...
    it('reports the offending Parameter cell, not just the row', () => {
        const level = chain('field_99', null);
        const result = collector();
        XlsxToJson.resolveGeoDependencies(worksheet, table, level, result);

        assert.equal(result.errors[0].cell, 'C6');
        assert.equal(result.errors[0].col, 'C');
    });

    it('ignores a dependency of another kind', () => {
        const level = chain(null, null);
        level[1].dependency = { on: 'field_1', kind: 'time' };
        const result = collector();
        XlsxToJson.resolveGeoDependencies(worksheet, table, level, result);

        assert.deepEqual(level[1].dependency, { on: 'field_1', kind: 'time' });
        assert.equal(result.errors.length, 0);
    });
    ...
    it('resolves a nested level against its own fields', () => {
        const nested = chain('field_1', 'field_2');
        const level = [
            field('outer_1', 'Continent', 'continent', null, 4),
            {
                name: 'sub',
                description: 'Sub',
        ...
        assert.deepEqual(nested[1].dependency, { on: 'field_1', kind: 'geo' });
        assert.deepEqual(nested[2].dependency, { on: 'field_2', kind: 'geo' });
        assert.equal(result.errors.length, 0);
    });

    it('does not let a nested field reach a parent one level up', () => {
        const nested = [field('inner_1', 'Country', 'country', 'outer_1', 6)];
        const level = [
            field('outer_1', 'Continent', 'continent', null, 4),
            {
                name: 'sub',
        ...
        assert.equal(nested[0].dependency, null);
        assert.equal(result.errors.length, 1);
        assert.equal(result.errors[0].text, 'Field "outer_1" not found.');
        assert.equal(result.errors[0].row, 6);
    });
});

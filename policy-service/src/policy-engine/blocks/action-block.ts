...
export class InterfaceDocumentActionBlock {
    ...
    async getData(user: PolicyUser): Promise<IPolicyGetData> {
        ...
        if (options.type === 'dropdown') {
            ...
            data.options = documents.map((e) => {
                return {
                    name: findOptions(e, options.name),
                    value: findOptions(e, options.value),
                }
            });
        }

        if (options.type === 'transformation') {
            const children = [];
    ...
    async setData(user: PolicyUser, document: IPolicyDocument, _, actionStatus): Promise<any> {
        ...
        let result: any = null;
        if (options.type === 'selector') {
            const option = this.findOptions(document, options.field, options.uiMetaData.options);
            if (option) {
        ...
        if (options.type === 'download') {
            ...
            const schemaObject = await PolicyUtils.loadSchemaByID(ref, options.schema);
            ...
            result = {
                fileName: options.filename || `${sensorDid}.config.json`,
                body: {
                    'url': options.targetUrl || process.env.MRV_ADDRESS,

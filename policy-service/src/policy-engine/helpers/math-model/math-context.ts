...
type TableFormulaPack = Record<string, { rows: any[]; columnKeys: string[] }>;
...
function tableMarkerHasRows(
    ...
    for (const source of sources) {
        ...
        if (!isTableValue(marker) ||
            !Array.isArray(marker.columnKeys) ||
            !marker.columnKeys.includes(columnKey)) {
            continue;
        }
        if (Array.isArray(marker.rows)) {
            continue;
        }
        if (typeof marker.fileId === 'string' && tablesPack?.[marker.fileId]) {
            continue;
        }
        return false;
    }
            ...
            for (const item of this.list) {
                if (item.type === MathItemType.LINK) {
                    ...
                    for (let index = 1; index < keys.length; index++) {
                        ...
                        column = resolveTableFormulaColumn(tableValue, columnKey, tablesPack);
                        if (column) {
                            if (!tableMarkerHasRows(tableValue, columnKey, tablesPack)) {

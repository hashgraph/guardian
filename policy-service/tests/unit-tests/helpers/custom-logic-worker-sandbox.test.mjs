...
// Threat reference: TM-006 (Critical) in the Hedera/Hashgraph Guardian Threat
// Model (April 2026). Mitigation M-005 requires the JS worker to fully isolate
// user-supplied code from the host policy-service process. The worker uses
// `node:vm` with a hardened sandbox; this suite asserts which escape vectors
// are actually contained and which remain exploitable today.
...
const POISON_ENV = 'POLICY_SVC_TM006_PROBE';
const POISON_VAL = 'tm006-secret-must-not-leak';
...
describe('CustomLogicBlock JS sandbox — TM-006 escape regression', function () {
    this.timeout(15000);
    ...
    describe('Prototype-chain escapes via passed-in outer-realm objects', () => {
        ...
        //
        // Tests in this block document the known-exploitable surface so a fix
        // (isolated-vm migration, or removing outer-realm intrinsic
        // assignments in custom-logic-worker.ts and letting the vm context
        // bootstrap its own) is gated by regression: when an escape is closed,
        // the corresponding test starts passing; if it ever re-opens, this
        // test fails again.
    ...
    describe('Execution timeout (TM-026 side benefit)', () => {

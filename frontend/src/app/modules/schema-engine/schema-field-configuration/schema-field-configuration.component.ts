...
export class SchemaFieldConfigurationComponent implements OnInit, OnDestroy {
    ...
    onEditExpression() {
        ...
        const helpContext: EditorHelpContext = {
            ...
            examples: [
                { label: 'Sum two fields', code: 'field1 + field2' },
                { label: 'Percentage', code: 'field1 / field2 * 100' },
                { label: 'Conditional value', code: 'field1 > 100 ? "high" : "low"' },
                { label: 'Nested condition', code: 'field1 > 100 ? "high" : field1 > 50 ? "medium" : "low"' },
                { label: 'Round to 2 decimals', code: 'Math.round(field1 / field2 * 100) / 100' },
                { label: 'Clamp value', code: 'Math.min(Math.max(field1, 0), 100)' },
                { label: 'String concatenation', code: '"ID-" + String(field1) + "-" + String(field2)' },
                { label: 'Nested schema field', code: 'subSchema.width * subSchema.height' },
                { label: 'With local variable (IIFE)', code: '(() => { const tax = field1 * 0.2; return field1 + tax; })()' },
                { label: 'Table column sum', code: 'table.col(myTable, "amount").reduce((s, v) => s + table.num(v), 0)' },
                { label: 'Table column average', code: 'table.col(myTable, "value").reduce((s, v) => s + table.num(v), 0) / table.rows(myTable).length' },
            ],

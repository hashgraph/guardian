...
| `pyodide.code` (exposes `run_js` → arbitrary Node host code)  | Cached entry evicted, stubbed in `sys.modules` _and_ on the parent `pyodide` package, blocked via import hook                                                                     |

# Policy Grid Actions API

The Policy Grid Actions API lets external integrators discover and execute workflow actions
(e.g. **Approve**, **Reject**) defined inside Guardian policy grid containers — without
reverse-engineering the internal policy block structure or touching any ephemeral block UUID.
...
All endpoints require a valid Guardian JWT bearer token in the `Authorization` header. The
caller must hold at least one of:

- `POLICIES_POLICY_EXECUTE`
- `POLICIES_POLICY_MANAGE`

Actions are further filtered by the caller's policy role — the API never returns or allows
execution of actions whose `requiredRoles` the caller does not hold.
...
|---|----------|-------------|
| 1 | [List Grids](policy-grid-actions-apis/list-grids.md) | Discover all grids visible to the caller |
| 2 | [List Actions](policy-grid-actions-apis/list-actions.md) | List executable actions on a specific grid |
| 3 | [Get Records](policy-grid-actions-apis/get-records.md) | Fetch paginated records from a grid |
| 4 | [Execute Action](policy-grid-actions-apis/execute-action.md) | Trigger a workflow action on a record |
